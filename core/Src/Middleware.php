...
//Используем трейт
    ...
    public function add($httpMethod, string $route, array $action):
    void
    {
        $this->middlewareCollector->addRoute($httpMethod, $route,
            $action);
    }
...
//Запуск всех middlewares для текущего маршрута
    ...
    public function go(string $httpMethod, string $uri, Request
                              $request): Request
    {
...
//Перебираем и запускаем их
        foreach ($routeMiddleware as $name => $class) {
            $args = explode(':', $name);
            $request = (new $class)->handle($request, $args[1]?? null) ??
                $request;
        }
        return $request;
    }
//Поиск middlewares по адресу
    private function getMiddlewaresForRoute(string $httpMethod,
                                            string $uri): array
    {
        $dispatcherMiddleware = new Dispatcher($this->middlewareCollector->getData());
        return $dispatcherMiddleware->dispatch($httpMethod, $uri)
        [1] ?? [];
    }
}