...
//Классы для использования внешнего маршрутизатора
    ...
    public static function add($httpMethod, string $route, array
                               $action): self
    {
        self::single()->routeCollector->addRoute($httpMethod, $route,
            $action);
...
//Конструктор скрыт. Вызывается только один раз
    ...
    {
        $this->routeCollector = new RouteCollector(new Std(), new
        MarkBased());
    }
